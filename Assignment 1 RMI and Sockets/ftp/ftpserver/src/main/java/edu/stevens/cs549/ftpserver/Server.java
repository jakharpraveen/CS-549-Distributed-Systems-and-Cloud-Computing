    ...
    private static class GetThread implements Runnable {
    	...
    	public void run () {
			...
			try {
				...
				throw new IllegalStateException("Trying to get a file", e);
			}
    ...
    private static class PutThread implements Runnable {
		...
		public void run() {
			try {
				...
				throw new IllegalStateException("Trying to put a file", e);
			}
    ...
    public void put (String file) throws IOException, FileNotFoundException, RemoteException {
		...
		try {
			if(mode == Mode.ACTIVE){
				...
				int cutoff = 0;				
				while ((cutoff = bis.read(buffer)) != -1) {
					fos.write(buffer);
				}
